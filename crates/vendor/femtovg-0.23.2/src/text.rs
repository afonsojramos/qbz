...
impl GlyphAtlas {
    ...
    // Renders the glyph into the atlas, retrying at the font's default
    // variation instance when the requested instance produces nothing.
    // Some variable fonts (e.g. the CFF2-based CJK system fonts on modern
    // macOS) fail to scale in swash at a non-default instance; without the
    // retry those glyphs get cached as missing and are never drawn, so any
    // non-regular weight renders blank (QBZ #543). Rendering the default
    // instance (regular weight) beats dropping the glyph entirely.
    #[allow(clippy::too_many_arguments)]
    fn render_glyph<T: Renderer>(
        ...
        let rendered = self.render_glyph_at_coords(
            canvas,
            font_size,
            line_width,
            mode,
        ...
        if rendered.is_some() || normalized_coords.is_empty() {
            return Ok(rendered);
        }
        log::debug!(
            "glyph {glyph_id} produced no image at a non-default variation instance; retrying at the default instance"
        );
        self.render_glyph_at_coords(
            canvas,
            font_size,
            line_width,
            mode,
    ...
    #[allow(clippy::too_many_arguments)]
    fn render_glyph_at_coords<T: Renderer>(
        &self,
        canvas: &mut Canvas<T>,
        font_size: f32,
        line_width: f32,
