...
pub fn create_collection(shared: bool) -> Collection {
    ...
    // On modern macOS the CoreText-based per-script fallback lookup can fail for
    // Han-script text: the family CoreText suggests (typically PingFang) is not
    // always resolvable in the scanned family map, which leaves CJK ideographs
    // unrendered while kana still resolves through Hiragino. Pre-register the
    // known system CJK families so the fallback map answers before the CoreText
    // query runs (`Collection::fallback_families` consults the map first).
    #[cfg(target_os = "macos")]
    {
        let find = |collection: &mut fontique::Collection,
                    names: &[&str]|
         -> Vec<fontique::FamilyId> {
            names.iter().filter_map(|name| collection.family_id(name)).collect()
        };
        let japanese = find(
            &mut collection,
            &["Hiragino Sans", "Hiragino Kaku Gothic ProN", "Hiragino Kaku Gothic Pro", "Osaka"],
        );
        let chinese =
            find(&mut collection, &["PingFang SC", "PingFang TC", "Heiti SC", "Songti SC"]);
        let korean = find(&mut collection, &["Apple SD Gothic Neo", "AppleGothic"]);
        let universal = find(&mut collection, &["Arial Unicode MS"]);

        // Text reaches the fallback query without a locale (no lang tags in the
        // UI), so every lookup lands on the language-agnostic default slot.
        // Bias Han towards Japanese: correct for kana-mixed text, still
        // readable for Chinese.
        let han_default: Vec<fontique::FamilyId> =
            japanese.iter().chain(&chinese).chain(&korean).chain(&universal).copied().collect();
        if !han_default.is_empty() {
            for script in ["Hani", "Hira", "Kana"] {
                collection.append_fallbacks(
                    fontique::FallbackKey::new(
                        fontique::Script::from_str_unchecked(script),
                        None,
                    ),
                    han_default.iter().copied(),
                );
            }
            // Language-tagged lookups do not fall through to the default slot,
            // so register the tracked languages as well in case a locale ever
            // reaches the query.
            for (lang, families) in [("ja", &japanese), ("zh", &chinese), ("ko", &korean)] {
                collection.append_fallbacks(
                    (fontique::Script::from_str_unchecked("Hani"), lang),
                    families.iter().chain(&universal).copied(),
                );
            }
        }
        let hangul: Vec<fontique::FamilyId> =
            korean.iter().chain(&universal).copied().collect();
        if !hangul.is_empty() {
            collection.append_fallbacks(
                fontique::FallbackKey::new(fontique::Script::from_str_unchecked("Hang"), None),
                hangul.iter().copied(),
            );
        }
    }
