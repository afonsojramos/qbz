...
impl<B: GraphicsBackend> FemtoVGRenderer<B> {
    ...
    fn internal_render_with_post_callback(
        ...
        window_inner
            .draw_contents(|components, post_render| -> Result<(), PlatformError> {
                ...
                let item_renderer = self::itemrenderer::GLItemRenderer::new(
                    &canvas,
                    &self.graphics_cache,
                    &self.layer_cache,
                    &self.texture_cache,
                ...
                let item_renderer = if partial_active {
                    ...
                    // (core item_rendering.rs) — with a multi-rect region they
                    // would re-blend over uncleared pixels in the gaps between
                    ...
                    post_render(&mut partial_renderer);
                    ...
                    let mut item_renderer = item_renderer;
                    if let Some(window_item_rc) = window_inner.window_item_rc() {
                        let window_item =
                            window_item_rc.downcast::<i_slint_core::items::WindowItem>().unwrap();
                        if let Brush::SolidColor(..) = window_item.as_pin_ref().background() {
    ...
    pub(crate) fn reset_canvas(&self, canvas: CanvasRc<B::Renderer>) {
        ...
        // the caches so layers re-render and images re-upload on the new
