...
impl GraphicsBackend for WGPUBackend {
    ...
    fn begin_surface_rendering(
        ...
        let frame = match surface.get_current_texture() {
            ...
            // no frame callback will arrive until it resumes). Skip the frame
            // and back off exponentially (250 ms → 2 s) so repeated redraw
            ...
            | wgpu::CurrentSurfaceTexture::Lost) => {
                ...
                match surface.get_current_texture() {
                    wgpu::CurrentSurfaceTexture::Success(t) => t,
                    // QBZ vendor patch (issue #558): if the retry is STILL stale
                    // the reconfigure hasn't caught up with the surface yet —
                    // this happens at startup under a non-default UI scale,
                    // where SLINT_SCALE_FACTOR changes the physical surface
                    // size and the first frames race the winit resize/
                    // scale-factor event that reconfigures the swapchain. Skip
                    // this frame and back off briefly (60 ms → 1 s); the next
                    // resize/redraw reconfigures with the correct dimensions
                    // and the following acquire succeeds.
                    _ => {
