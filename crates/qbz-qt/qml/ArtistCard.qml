            ...
            Rectangle {
                ...
                Row {
                    ...
                    CardOverlayButton {
                        ...
                        primary: true
                        anchors.verticalCenter: parent.verticalCenter
                        onClicked: QbzBridge.playArtistCard(root.item.id)
                    }
                    CardOverlayButton {
                        id: agMore
                        name: "ellipsis"
                ...
                CardMenu {
                    ...
                    entries: {
                        ...
                        if (root.showFollow) m.push({
                            "label": root.item.following ? QbzBridge.tr("Following", QbzBridge.trRev) : QbzBridge.tr("Follow", QbzBridge.trRev),
                            "icon": root.item.following ? "check" : "user-plus",
                            "action": "follow" })
                        m.push({ "label": QbzBridge.tr("Not interested", QbzBridge.trRev), "icon": "thumbs-down", "action": "not-interested" })
                        return m
                    }
                    onPicked: function (a) {
                        if (a === "open") QbzBridge.openArtist(root.item.id)
                        else if (a === "play") QbzBridge.playArtistCard(root.item.id)
