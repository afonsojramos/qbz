...
Rectangle {
    ...
    readonly property bool isActive: QbzBridge.npTrackId === (item.id || "")
    ...
    Row {
        ...
        Item {
            ...
            Rectangle {
                ...
                height: 28
                radius: 14
                color: root.isActive && QbzBridge.npPlaying ? "transparent" : "#3dffffff"
                border.width: root.isActive && QbzBridge.npPlaying ? 1.5 : 0
                border.color: theme.accent
                QbzIcon {
                    anchors.centerIn: parent
                    name: root.isActive && QbzBridge.npPlaying ? "pause" : "play-fill"
                    width: 14
                    height: 14
                    tintName: root.isActive && QbzBridge.npPlaying ? "accent" : "primary"
                }
            }
        }
    ...
    CardMenu {
        ...
        entries: {
            ...
            if (root.menuShowLater) m.push({ "label": QbzBridge.tr("Play later", QbzBridge.trRev), "icon": "list-plus", "action": "later" })
            m.push({ "label": QbzBridge.tr("Add to queue", QbzBridge.trRev), "icon": "list-end", "action": "queue" })
            if (root.menuShowGoTo && root.item.artistId) m.push({ "label": QbzBridge.tr("Go to artist", QbzBridge.trRev), "icon": "user", "action": "go-artist" })
            if (root.menuShowGoTo && root.item.albumId) m.push({ "label": QbzBridge.tr("Go to album", QbzBridge.trRev), "icon": "disc", "action": "go-album" })
            if (root.menuShowFavorite) m.push({
                "label": root.item.isFavorite ? QbzBridge.tr("Remove from Library", QbzBridge.trRev) : QbzBridge.tr("Add to Library", QbzBridge.trRev),
                "icon": root.item.isFavorite ? "heart-filled" : "heart", "action": "favorite" })
            if (root.menuShowRemove) m.push({ "label": QbzBridge.tr("Remove from playlist", QbzBridge.trRev), "icon": "trash-2", "action": "remove" })
            return m
        }
        onPicked: function (a) {
    ...
    MouseArea {
        ...
        onPositionChanged: function (mouse) {
            ...
            if (!root.dragging
                ...
                QbzBridge.dragStart(root.item.id, root.item.title || "",
                    (root.item.artist || "") + " · " + (root.item.album || ""), g.x, g.y)
            }
            if (root.dragging) QbzBridge.dragMove(g.x, g.y)
        }
