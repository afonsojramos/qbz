...
mod tests {
    ...
    fn fresh_store(name: &str) -> (std::path::PathBuf, LibraryPreferencesStore) {
        let dir = unique_test_dir(name);
        let store = LibraryPreferencesStore::new_at(&dir).expect("open store in temp dir");
        (dir, store)
    }
    ...
    fn legacy_json_without_folders_view_mode_defaults_to_flat() {
        ...
        let prefs: LibraryPreferences = serde_json::from_str(legacy).expect("parse legacy json");
        assert_eq!(prefs.folders_view_mode, FoldersViewMode::Flat);
    }
    ...
    fn round_trip_with_tree_set() {
        ...
        let parsed: LibraryPreferences = serde_json::from_str(&json).expect("deserialize");
    ...
    fn folders_view_mode_str_round_trip() {
        ...
        assert_eq!(FoldersViewMode::from_str("garbage"), FoldersViewMode::Flat);
    }

    #[test]
    fn fresh_store_returns_default_folders_view_mode() {
        let (dir, store) = fresh_store("library-default-mode");
    ...
    fn set_folders_view_mode_persists_on_empty_table() {
        let (dir, store) = fresh_store("library-mode-empty");
        store
    ...
    fn set_folders_view_mode_updates_existing_row() {
        let (dir, store) = fresh_store("library-mode-update");
    ...
    fn save_preferences_persists_folders_view_mode() {
        let (dir, store) = fresh_store("library-save-mode");
        let mut prefs = LibraryPreferences::default();
        prefs.folders_view_mode = FoldersViewMode::Tree;
    ...
    fn fresh_store_returns_none_folders_tree_sidebar_width() {
        let (dir, store) = fresh_store("library-default-width");
        let prefs = store.get_preferences().expect("get prefs");
    ...
    fn set_folders_tree_sidebar_width_persists_on_empty_table() {
        let (dir, store) = fresh_store("library-width-empty");
        store
    ...
    fn set_folders_tree_sidebar_width_updates_existing_row() {
        let (dir, store) = fresh_store("library-width-update");
