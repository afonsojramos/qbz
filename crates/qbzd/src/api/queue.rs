...
/// failure, nothing partially added).
pub fn add(state: &ApiState, body: &Value) -> Response<Cursor<Vec<u8>>> {
    if let Some(resp) = auth_gate(state) {
        return resp;
    }

    let track_ids: Vec<u64> = match body.get("track_ids").and_then(|v| v.as_array()) {
        Some(arr) if !arr.is_empty() => arr.iter().filter_map(|v| v.as_u64()).collect(),
        _ => {
            return err_json(
                400,
                "bad_request",
                "add requires a non-empty 'track_ids' array",
                "body: {\"track_ids\": [176544872]}",
            )
        }
    };
    let position = body.get("position").and_then(|v| v.as_str()).unwrap_or("end");
...
/// playing track — the verbatim hint below is quoted §3.3.15).
pub fn remove(state: &ApiState, body: &Value) -> Response<Cursor<Vec<u8>>> {
    let index = match body.get("index").and_then(|v| v.as_u64()) {
        ...
        None => {
            return err_json(
                400,
                "bad_request",
                "remove requires an 'index' field",
                "body: {\"index\": 3}",
            )
        }
    };
    ...
    match state.rt.block_on(state.runtime.core().remove_track(index)) {
        ...
        // Unreachable given the bounds check above (queue cannot shrink
        // between the read and this call — the API thread is single-threaded,
        // §"one serving thread" in api/mod.rs), but a defensive 404 beats a
        // 500 if that invariant ever breaks.
        None => err_json(
            404,
            "not_found",
            &format!("queue index {index} is out of range"),
