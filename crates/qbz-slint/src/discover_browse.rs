...
//! returned item count and stops once `has_more` is false. Reuses the
//! Discover home mappers (`crate::home::map_album` / `card_to_item`) so
//! the cards carry the same genre + localized release date as the home
//! carousels.
//!
//! Header tools (mirroring Tauri's DiscoverBrowseView): a client-side
//! search filter over the loaded albums (disables load-more while
//! active), the shared genre filter (re-fetches from offset 0 with the
//! selected genre ids), and a grid/list view toggle.
    ...
    handle.spawn(async move {
            ...
            .await
        {
            Ok(mut data) => {
                ...
                let next_offset = cards.len() as i32;
                ...
                let _ = weak.upgrade_in_event_loop(move |w| {
                    ...
                    state.set_next_offset(next_offset);
    ...
    handle.spawn(async move {
            ...
            .await
        {
            Ok(mut data) => {
                ...
                let _ = weak.upgrade_in_event_loop(move |w| {
                    append_albums(&w, cards, has_more);
                });
                artwork::spawn_loads(jobs, weak.clone(), image_cache.clone());
            }
            Err(e) => {
...
/// offset by the new item count and updating has-more. UI thread only.
fn append_albums(window: &AppWindow, cards: Vec<CardData>, has_more: bool) {
