...
impl Service {
    pub(crate) fn from_xml(node: Node<'_, '_>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            scpd_endpoint: parse_service_endpoint(scpd_endpoint)?,
            control_endpoint: parse_service_endpoint(control_endpoint)?,
            event_sub_endpoint: parse_service_endpoint(event_sub_endpoint)?,
        })
    }
...
// QBZ #745: preserve the wire paths accepted by http 1.4.0 without downgrading
// http for the whole application. Valid PathAndQuery values take the original
// parser unchanged. Empty paths previously represented `/`. http 1.4.0 sent
// relative endpoints without a leading slash even in the HTTP request target;
// http 1.5.0 rejects them entirely. Normalize only those rejected endpoints to
// valid origin-form paths. Do not change URLBase/description-directory handling
// or any endpoint already accepted by the released 2.1.0 parser.
fn parse_service_endpoint(node: Node<'_, '_>) -> Result<PathAndQuery> {
    let text = node.text().unwrap_or("");
    if text.is_empty() {
        return Ok(PathAndQuery::from_static("/"));
    }
    match text.parse::<PathAndQuery>() {
        Ok(path) => Ok(path),
        Err(error) if !text.starts_with('/') => {
            // Re-parse after adding only the missing slash: invalid characters
            // and the http crate's size limit remain enforced.
            format!("/{text}").parse().map_err(|_| Error::invalid_response(error))
        }
        Err(error) => Err(Error::invalid_response(error)),
    }
}
