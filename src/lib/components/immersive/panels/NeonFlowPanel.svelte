<script lang="ts">
  ...
  function getLaserBandHue(bandIdx: number, ringIndex: number, spokeIdx: number, timeMs: number): number {
    // Arcade extreme mapping:
    // bass stays in red/orange family, highs stay in electric blue/cyan, mids bridge with vivid jumps.
    const rapid = timeMs * 0.18;
    const flicker = Math.sin(timeMs * 0.011 + ringIndex * 2.7 + spokeIdx * 1.5) * 7;
    const chaos = Math.cos(timeMs * 0.007 + bandIdx * 1.3 + spokeIdx * 0.8) * 9;

    if (bandIdx <= 3) {
      const bassPalette = [2, 10, 18, 26];
      const base = bassPalette[Math.max(0, Math.min(bassPalette.length - 1, bandIdx))];
      return wrapHue(base + rapid * 1.35 + flicker + chaos * 0.5);
    }

    if (bandIdx >= 10) {
      const highPalette = [198, 210, 222, 234, 246, 258];
      const highIdx = Math.max(0, Math.min(highPalette.length - 1, bandIdx - 10));
      return wrapHue(highPalette[highIdx] + rapid * 1.48 + flicker * 0.75 + chaos * 0.45);
    }

    const midPalette = [54, 72, 102, 132, 286, 316];
    const midIdx = Math.max(0, Math.min(midPalette.length - 1, bandIdx - 4));
    return wrapHue(midPalette[midIdx] + rapid * 1.25 + flicker * 1.1 + chaos);
  }
