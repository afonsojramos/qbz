<script lang="ts">
  ...
  interface TopQCacheEntry {
    localDate: string;
    result: DynamicSuggestResponse;
    cachedAtIso: string;
  }

  const TOPQ_CACHE_KEY = 'qbz-topq-cache-v1';
  const TOPQ_TARGET_SIZE = 50;
  const TOPQ_ANALYSE_TRACKS = 9;
  const TOPQ_MAX_PLAYLISTS = 30;
  const TOPQ_PICK_PLAYLISTS = 20;
  ...
  let emptyPlaylists = $state(false);
  ...
  function readCache(localDate: string): TopQCacheEntry | null {
    const raw = getUserItem(TOPQ_CACHE_KEY);
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw) as TopQCacheEntry;
  ...
  function writeCache(entry: TopQCacheEntry): void {
    setUserItem(TOPQ_CACHE_KEY, JSON.stringify(entry));
  }
  ...
  async function buildSeeds(): Promise<{
    ...
    // Get all playlist stats and find most-played
    const allStats = await invoke<PlaylistStats[]>('v2_playlist_get_all_stats');
    const playedStats = allStats
      .filter(stat => stat.play_count > 0)
      .sort((a, b) => b.play_count - a.play_count)
      .slice(0, TOPQ_MAX_PLAYLISTS);

    if (playedStats.length === 0) {
      return { listenedTrackIds: [], tracksToAnalyse: [], limit: TOPQ_TARGET_SIZE };
    }

    // Randomly pick up to TOPQ_PICK_PLAYLISTS
    const selectedStats = shuffleArray(playedStats).slice(0, TOPQ_PICK_PLAYLISTS);

    // Fetch track IDs from each selected playlist
    const playlistResults = await Promise.all(
      selectedStats.map(stat =>
        invoke<PlaylistWithTrackIds>('v2_get_playlist_track_ids', {
          playlistId: stat.qobuz_playlist_id
        }).catch(() => null)
      )
    );

    // Merge + dedupe all track IDs
    const allTrackIds: number[] = [];
    for (const plResult of playlistResults) {
      if (plResult && plResult.track_ids) {
        allTrackIds.push(...plResult.track_ids);
      }
    }

    const merged = dedupeTrackIds(allTrackIds);
    if (merged.length === 0) {
      return { listenedTrackIds: [], tracksToAnalyse: [], limit: TOPQ_TARGET_SIZE };
    }

    // Pick spread for analysis
    const analysedSeedIds = pickSpread(merged, TOPQ_ANALYSE_TRACKS);
    ...
    const tracksToAnalyse = analysed
      ...
      .slice(0, TOPQ_ANALYSE_TRACKS);

    const limit = Math.max(1, TOPQ_TARGET_SIZE - tracksToAnalyse.length);
  ...
  async function generateTopQ(cacheMode: 'none' | 'read-write' = 'none'): Promise<void> {
    loading = true;
    error = null;
  ...
  function buildQueueTracks(queueItems: DynamicSuggestTrack[]) {
    return queueItems
      .filter(trk => {
...
<div class="topq-view">
    ...
    <div class="metadata">
      ...
      <h1 class="playlist-title">
        ...
        <span class="info-wrapper">
          ...
          {#if showAlgoTooltip}
            <span class="info-tooltip">{$t('topMixes.algorithmInfo')}</span>
          {/if}
        </span>
      </h1>
      <p class="playlist-description">{@html $t('topMixes.cardDesc')}</p>
      ...
      <div class="actions">
        ...
        <button class="action-btn-circle" onclick={() => generateTopQ('none')} disabled={loading} title={$t('actions.refresh')}>
          <RefreshCw size={18} />
        </button>
        <button
  ...
  {#if emptyPlaylists && !loading}
    <div class="empty">{$t('topMixes.emptyPlaylists')}</div>
  {:else}
